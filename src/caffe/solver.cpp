...
(4)SGDSolver::ApplyUpdate函数的具体实现。前面三个部分都属于基类的，最后一个是SGDSolver这个子类的，
如果用户想要实现自己的Solver类，也应该类似地去继承基类，并实现自己的ApplyUpdate函数，在代码的末尾通过register宏完成注册
，便可以被成功的调用。
*/
namespace caffe {
...
//训练网络只能有一个,在InitTrainNet()函数中首先会设置一些基本参数，包括设置网络的状态为TRAIN，确定训练网络只有一个等，
//然会会通过下面这条语句新建了一个Net对象: net_.reset(new Net<Dtype>(net_param));
template <typename Dtype>
void Solver<Dtype>::InitTrainNet() {
  ...
  if (Caffe::root_solver()) {
    //shared_ptr的reset函数，不带参数的是将引用计数减1，停止对指针的共享，除非引用计数为0，否则不会发生删除操作。
...
void Solver<Dtype>::Step(int iters) {
  ...
  // 输出的loss为前average_loss次的loss的平均值，在solver.prototxt里设置，默认为1，
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  // 迭代结束或者遇到系统信号提前结束后，判断是否需要在训练结束之后snapshot.这个可以在solver.prototxt里设置
...
void Solver<Dtype>::Test(const int test_net_id) {
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    Dtype iter_loss;
    //result存放网络最后一层的weight（就是最后一层的blob_)
