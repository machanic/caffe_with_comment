...
void SGDSolver<Dtype>::ClipGradients() {
  ...
  const Dtype l2norm_diff = std::sqrt(sumsq_diff); //计算二范数
  if (l2norm_diff > clip_gradients) { //二范数大于clip_gradients
...
void SGDSolver<Dtype>::ApplyUpdate() {
  ...
  for (int param_id = 0; param_id < this->net_->learnable_params().size();
       ++param_id) {
    // 将第param_id个参数的梯度除以iter_size，这一步的作用是保证 实际的batch_size=iter_size*设置的batch_size
    Normalize(param_id);
...
void SGDSolver<Dtype>::ComputeUpdateValue(int param_id, Dtype rate) {
  ...
  switch (Caffe::mode()) {
  case Caffe::CPU: {
     // 关于SGD的公式参考caffe官网tutorial的Solver部分：http://caffe.berkeleyvision.org/tutorial/solver.html
     // history_存储了上一次的梯度，下面这个函数：
     // history_ = learning_rate*diff_ + momentum*history
    caffe_cpu_axpby(net_params[param_id]->count(), local_rate,
              net_params[param_id]->cpu_diff(), momentum,  //net_params[param_id]->cpu_diff便是损失函数loss关于weight的负梯度
              history_[param_id]->mutable_cpu_data()); //当你想读取数据的时候请使用cpu_data，当然想修改数据的时候请你使用mutable_cpu_data。
    // 把当前的梯度拷贝给参数Blob的diff_
    caffe_copy(net_params[param_id]->count(),
        history_[param_id]->cpu_data(), //当你想读取数据的时候请使用cpu_data，当然想修改数据的时候请你使用mutable_cpu_data。
        net_params[param_id]->mutable_cpu_diff());
    break;
  }
  case Caffe::GPU: {
