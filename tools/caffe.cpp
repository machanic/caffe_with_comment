...
//see http://blog.csdn.net/u014114990/article/details/47747025
...
**/
  caffe::SignalHandler signal_handler(
        ...
        //关键的一句话
        /**
        总体的流程大概就是新建一个Solver对象，然后调用Solver类的构造函数，然后在Solver的构造函数中又会新建Net类实例，
        在Net类的构造函数中又会新建各个Layer的实例,
        一直具体到设置每个Blob,大概就介绍完了网络初始化的工作，当然里面还有很多具体的细节，但大概的流程就是这样。
        **/
  shared_ptr<caffe::Solver<float> >
      solver(caffe::SolverRegistry<float>::CreateSolver(solver_param));
      //这一句的CreateSolver调用了solver_factory.hpp里的SolverRegistry类的函数
      //由于C++多态的特性，尽管solver是一个指向基类Solver类型的指针，通过solver这个智能指针来调用各个成员函数会调用到各个子类(SGDSolver等)的函数

  solver->SetActionFunction(signal_handler.GetActionFunction());//设置回掉函数
